...
if [ $IS_ENTERPRISE = "True" ]; then
    ...
    GITHUB_RESPONSE=$(sudo git clone --depth 1 --branch $OE_VERSION https://github.com/ShaheenHossain/dosyt14_ent_unlimit "$OE_HOME/enterprise/addons" 2>&1)
    while [[ $GITHUB_RESPONSE == *"Authentication"* ]]; do
        echo "------------------------WARNING------------------------------"
        echo "Your authentication with Github has failed! Please try again."
        printf "In order to clone and install the dosyt enterprise version you \nneed to be an offical dosyt partner and you need access to\nhttp://github.com/dosyt/enterprise.\n"
...
if [ $INSTALL_NGINX = "True" ]; then
  ...
  cat <<EOF > ~/etworkent1454
  server {
  listen 80;
  # set proper server name after domain set
  server_name $WEBSITE_NAME;
  # Add Headers for etwork proxy mode
...
EOF
  ...
  sudo mv ~/etworkent1454 /etc/nginx/sites-available/
  sudo ln -s /etc/nginx/sites-available/dosytent1454 /etc/nginx/sites-enabled/dosytent1454
  sudo rm /etc/nginx/sites-enabled/default
  sudo service nginx reload
  sudo su root -c "printf 'proxy_mode = True\n' >> /etc/${OE_CONFIG}.conf"
  echo "Done! The Nginx server is up and running. Configuration can be found at /etc/nginx/sites-available/dosytent1454"
else
...
echo -e "* Starting etwork Service"
sudo su root -c "/etc/init.d/$OE_CONFIG start"
echo "-----------------------------------------------------------"
echo "Done! The etwork server is up and running. Specifications:"
echo "Port: $OE_PORT"
